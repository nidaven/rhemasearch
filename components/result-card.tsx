...
function ResultCard({ title, summary, url, image_url, date, snippets }: SermonProps) {
    ...
    return (
        <Card className="grid justify-items-stretch bg-transparent mb-8">
            <div className="grid grid-cols-3 gap-3 max-h-36 pt-4 px-6">
            ...
            <Collapsible className="grid grid-cols-auto w-full" open={open} onOpenChange={setOpen}>
                <CollapsibleTrigger className="grow shrink-0 p-1 mx-2 mt-4 text-center text-sm justify-center flex flex-row items-center bg-cyan-200/25 text-cyan-600/80">
                    {open ? 'Hide Summary' : 'Show Summary'}
                    {open ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                </CollapsibleTrigger>                
                <CollapsibleContent className="max-h-28 pt-2 mt-2 overflow-y-auto text-sm text-cyan-600/80 p-4">
                    {summary}
                </CollapsibleContent>
            </Collapsible>
