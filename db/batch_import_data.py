...
batch_size = 30
FILE_NAME = 'services_spotify.json'
#pass record index
def add_sermon_to_batch(sermon_record):
    record = {
        "title" : sermon_record["episode_name"],
        "duration" : sermon_record["duration"],
        "url" : sermon_record["spotify_url"],
        "summary" : sermon_record["text_summary"],
        "image_url" : sermon_record["image_url"],
        "date" : sermon_record["service_date"]

    }
    return {
        "record" : record,
        "class" : "Sermon",
        "uuid": str(uuid5(NAMESPACE_DNS, sermon_record["episode_name"].replace(" ","_")+str(len(sermon_record["text_summary"]))))
    }
...
def import_data(client: weaviate.Client, batch_size, length=None):
    ...
    sermons_info = json.load(open(FILE_NAME))
    ...
    for idx in episode_idxs:
        ...
        for item in range(0, length_of_segment):
            ...
            client.batch.add_data_object(segment_object['record'], segment_object['class'], segment_object['uuid'], segment_object['vector'])
            client.batch.add_reference(segment_object['uuid'], segment_object['class'], 'fromSermon', sermon_object['uuid'], sermon_object["class"])
             
            segment_count += 1
            if segment_count == batch_size:
